...
function stugel_rw_vs_lt(str) {
    if (jQuery("#Rich_Web_" + str + "_LT_T").val() == "Type 1") {
        jQuery(".rw_text_color").hide();
        jQuery(".rw_text_color1").show();
    } else if (jQuery("#Rich_Web_" + str + "_LT_T").val() == "Type 2") {
        jQuery(".rw_text_color").hide();
        jQuery(".rw_text_color2").show();
    } else if (jQuery("#Rich_Web_" + str + "_LT_T").val() == "Type 3") {
        jQuery(".rw_text_color").hide();
        jQuery(".rw_text_color3").show();
...
function Rich_Web_VSlider_Edit_Option(rich_web_Slider_ID) {
    ...
    var data = {
        ...
        action: 'rich_web_VS_Edit_Option',
        foobar: rich_web_Slider_ID,
    };
    jQuery.post(ajaxurl, data, function(response) {
        ...
        for (i = 0; i < data.length; i++) {
            for (var key in data[i][0]) {
                if (data[i][0][key] == 'true' || data[i][0][key] == 'on' || data[i][0][key] == 'On') { jQuery("#" + key).attr('checked', true); } else if (data[i][0][key] == 'false' || data[i][0][key] == '' || data[i][0][key] == 'none' || data[i][0][key] == 'off' || data[i][0][key] == 'Off') { jQuery("#" + key).attr('checked', false); } else { jQuery("#" + key).val(data[i][0][key]); }
            }
        }
...
function Rich_Web_VSlider_Copy_Option(rich_web_Slider_ID) {
    ...
    var data = {
        rwvs_nonce: object.rwvs_nonce,
        action: 'rich_web_VS_Copy_Option',
        foobar: rich_web_Slider_ID,
    };
    jQuery.post(ajaxurl, data, function(response) {
...
function Rich_Web_VS_Name_Available() {
    ...
    var data = {
        rwvs_nonce: object.rwvs_nonce,
        action: 'Rich_Web_VS_Name_Available',
...
jQuery(document).ready(function() {
    if (!navigator.onLine) {
        jQuery('.Rich_Web_VS_Opt_Content img').each(function() {
            jQuery(this).attr('src', object.rwvs_no_conect);
        });
    }
});