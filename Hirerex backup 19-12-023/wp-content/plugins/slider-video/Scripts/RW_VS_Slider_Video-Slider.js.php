...
<script>
	<!--
	Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_slider_init = function() {
	var Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_SlideshowTransitions = [
	...
	var y="<?php echo esc_attr($Rich_Web_VSlider_Eff[0]->Rich_Web_VTVS_AP);?>";
	var trueFalse;
	if(y==""){ trueFalse=false; }else{ trueFalse=true; }
	var Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_options = {
		$AutoPlay: trueFalse,
		$AutoPlaySteps: <?php echo esc_attr($Rich_Web_VSlider_Eff[0]->Rich_Web_VTVS_APS);?>,
		$Idle: <?php echo esc_attr($Rich_Web_VSlider_Eff[0]->Rich_Web_VTVS_PT*1000);?>,
		$SlideDuration: <?php echo esc_attr($Rich_Web_VSlider_Eff[0]->Rich_Web_VTVS_CS);?>,
		$SlideshowOptions: {
			$Class: $JssorSlideshowRunner$,
			$Transitions: Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_SlideshowTransitions,
			$TransitionsOrder: 1
		},
		$ArrowNavigatorOptions: {
			$Class: $JssorArrowNavigator$,
			$Steps: <?php echo esc_attr($Rich_Web_VSlider_Eff[0]->Rich_Web_VTVS_ArrSt);?>
		},
	...
	var Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_slider = new $JssorSlider$("Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>", Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_options);
	function ScaleSlider() {
		var refSize = Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_slider.$Elmt.parentNode.clientWidth;
		if (refSize)
		{
			refSize = Math.min(refSize, <?php echo esc_attr($Rich_Web_VSlider_Eff[0]->Rich_Web_VTVS_H*16/9+240);?>);
			refSize = Math.max(refSize, 240);
			Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_slider.$ScaleWidth(refSize);
		}
...
<script>
	<!--	
	jQuery("#Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>").parent().closest('div').css({'padding':'0','display':'block','overflow':'hidden','box-sizing':'border-box'});
	jQuery(".elementor-element").css({'width':'100%','overflow':'hidden','box-sizing':'border-box'});
	jQuery(".Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_Main").closest("span").css({'display':'block'});
	...
	function Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_Play_Video(Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_Play_Video_Src)
	{
		jQuery('.Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_Iframe_Div').css('display','inline');
		jQuery('#Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_Iframe').attr('src',Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_Play_Video_Src+'?autoplay=1&rel=0&amp');
	}
	function Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_Stop_Video()
	{
		jQuery('.Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_Iframe_Div').css('display','none');
		jQuery('#Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_Iframe').attr('src','');
	}
	jQuery("#RW_Load_VSVT_Navigation<?php echo esc_html($Rich_Web_VSlider_ID); ?>").css("max-height",Math.floor(jQuery("#RW_Load_VSVT_Navigation<?php echo esc_html($Rich_Web_VSlider_ID); ?>").width()*50/100));
	jQuery(window).resize(function(){
		jQuery("#RW_Load_VSVT_Navigation<?php echo esc_html($Rich_Web_VSlider_ID); ?>").css("max-height",Math.floor(jQuery("#RW_Load_VSVT_Navigation<?php echo esc_html($Rich_Web_VSlider_ID); ?>").width()*50/100));
	})
	var array_vsvt<?php echo esc_html($Rich_Web_VSlider_ID); ?>=[];
	jQuery(".image_rw_data<?php echo esc_attr($Rich_Web_VSlider_ID);?>").each(function(){
		if( jQuery(this).attr("data-rwimg") != "" ) {
			array_vsvt<?php echo esc_html($Rich_Web_VSlider_ID); ?>.push(jQuery(this).attr("data-rwimg"));
			jQuery(this).attr("src",jQuery(this).attr("data-rwimg"))
		}
	})
	var y_vsvt<?php echo esc_html($Rich_Web_VSlider_ID); ?>=0;
	for(i=0;i<array_vsvt<?php echo esc_html($Rich_Web_VSlider_ID); ?>.length;i++){
		jQuery("<img class='image_rw_data<?php echo esc_html($Rich_Web_VSlider_ID); ?>' />").attr('src', array_vsvt<?php echo esc_html($Rich_Web_VSlider_ID); ?>[i]).on("load",function(){
			y_vsvt<?php echo esc_html($Rich_Web_VSlider_ID); ?>++;
			if(y_vsvt<?php echo esc_html($Rich_Web_VSlider_ID); ?> == array_vsvt<?php echo esc_html($Rich_Web_VSlider_ID); ?>.length){
				jQuery("#Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>").fadeIn(1000);
				jQuery("#RW_Load_VSVT_Navigation<?php echo esc_html($Rich_Web_VSlider_ID); ?>").remove();
			}
		})
	}
	//-->	
...
<script>
	Rich_Web_VSVT<?php echo esc_html($Rich_Web_VSlider_ID); ?>_slider_init();
	jQuery(document).ready(function(){
		jQuery('div[data-u="prototype"]').each(function(index,elem){
			jQuery(this).children('.w').children('img').attr('src',array_vsvt<?php echo esc_html($Rich_Web_VSlider_ID); ?>[index]).attr('style',`display:inline-block !important;visibility:visible !important;`)
		});
	});
</script>